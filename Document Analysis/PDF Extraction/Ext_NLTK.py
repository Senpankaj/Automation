...
def preprocess_text_nltk(text):
    ...
    for paragraph in paragraphs:
        ...
        # Iterate through sentences to extract headings and content
        for sentence in sentences:
            # Detect and extract headings using regular expression
            if re.match(r'^[0-9]+\.[0-9]+\.?\s', sentence):  # Matches subsections like 2.1 or 2.1.
                processed_text.append(f"## {sentence.strip()}")
            elif re.match(r'^[0-9]+\.\s', sentence):  # Matches sections like 2.
                processed_text.append(f"## {sentence.strip()}")
