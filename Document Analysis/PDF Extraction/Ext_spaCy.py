...
def preprocess_text_spacy(text):
    ...
    for token in doc:
        # Check if token is a heading (identified by being at the start of a sentence and capitalized)
        if token.is_sent_start and token.text.istitle() and token.text.strip('.').isalpha():
            # Append current section to processed text if it exists
            if current_heading:
                processed_text.append(current_heading)
                processed_text.append("")
            
            # Initialize new section heading
            current_heading = f"# {token.text.strip()}"
        else:
            # Append token text to current section content
            if current_heading:
                processed_text.append(token.text.strip())
